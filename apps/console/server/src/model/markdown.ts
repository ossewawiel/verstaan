...
// A small Markdown renderer for the project's own documents, ported from
// tools/console/src/markdown.mjs. Escapes everything first, then recognises: frontmatter
// (stripped, returned), headings, paragraphs, lists, tables, fenced code, blockquotes, rules,
// bold, italics, inline code, links. Enough for this repo; not a general renderer.
...
type LinkMap = ((href: string) => string) | undefined;
type CodeLink = ((code: string) => string | null) | undefined;

function inline(s: string, linkMap: LinkMap, codeLink: CodeLink): string {
...
export function render(md: string, { linkMap, codeLink }: RenderOptions = {}): RenderResult {
  ...
  function para(buf: string[]) {
    if (buf.length) {
      out.push(`<p>${inline(buf.join(' '), linkMap, codeLink)}</p>`);
      buf.length = 0;
    }
  }

  function list(startIndent: number) {
    ...
    while (i < lines.length) {
      ...
      i += 1;
      const cont: string[] = [];
      while (i < lines.length && /^\s+\S/.test(lines[i]) && !/^\s*([-*]|\d+[.)])\s/.exec(lines[i])) {
  ...
  while (i < lines.length) {
    ...
    if ((m = /^(#{1,6})\s+(.*?)\s*#*\s*$/.exec(line))) {
      ...
      out.push(`<h${level} id="${id}">${inline(text, linkMap, codeLink)}</h${level}>`);
      i += 1;
      continue;
    }
    if (/^```/.test(line)) {
    ...
    if (/^\s*\|.*\|\s*$/.test(line) && i + 1 < lines.length && /^\s*\|?\s*:?-{2,}/.test(lines[i + 1])) {
      ...
      out.push(
        '<div class="tablewrap"><table><thead><tr>' +
          head.map((h) => `<th>${inline(h, linkMap, codeLink)}</th>`).join('') +
          '</tr></thead><tbody>' +
          rows.map((r) => '<tr>' + r.map((c) => `<td>${inline(c, linkMap, codeLink)}</td>`).join('') + '</tr>').join('') +
          '</tbody></table></div>',
      );
      continue;
    }
