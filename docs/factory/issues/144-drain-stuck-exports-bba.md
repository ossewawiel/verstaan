...
effort: medium
checkpoint: null
commit: efdfe2f08aaf686a52fd94a6e818e8e87a7a4ba3
worktree: null
github_issue: 160
---
## What

Issue 143 drained Serbian: `python -m tools.mirror retry --language srp` landed every `srp`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `bba` (Baatonum), the next answer from its fallback rule once the stuck language ahead of
...
After this quest, Baatonum is drained the same way Serbian was: `python -m tools.mirror retry
--language bba --passes 3 --pause-seconds 600` runs against the live archive until `bba` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Baatonum `stuck` count before
and after. At its close, `/quest` writes issue 145 for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `bba`.
- One live `retry --language bba` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Baatonum `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 145 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Baatonum in this quest's live run. Changing
...
- [ ] Baatonum is drained as far as three passes allow: each pass's summary line and the
      Baatonum `stuck` count before and after are in the report, and the landed zips are in
      the work commit.
- [ ] Issue 145 for the next language is on `main`, or the report records that `stuck --next`
