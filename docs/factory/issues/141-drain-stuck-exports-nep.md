...
Issue 140 drained Hungarian: `python -m tools.mirror retry --language hun` landed every `hun`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `nep` (Nepali), the next answer from its fallback rule once the stuck language ahead of
...
After this quest, Nepali is drained the same way Hungarian was: `python -m tools.mirror retry
--language nep --passes 3 --pause-seconds 600` runs against the live archive until `nep` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Nepali `stuck` count before
and after. At its close, `/quest` writes issue 142 for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `nep`.
- One live `retry --language nep` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Nepali `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 142 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Nepali in this quest's live run. Changing
