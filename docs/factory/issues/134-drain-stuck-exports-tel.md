...
checkpoint: null
commit: null
worktree: .worktrees/quest-134-drain-stuck-exports-tel
github_issue: 135
---
## What

Issue 133 drained Ukrainian: `python -m tools.mirror retry --language ukr` landed every `ukr`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `tel` (Telugu), the next answer from its fallback rule once the stuck language ahead of it
...
After this quest, Telugu is drained the same way Ukrainian was: `python -m tools.mirror retry
--language tel --passes 3 --pause-seconds 600` runs against the live archive until `tel` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Telugu `stuck` count before and
after. At its close, `/quest` writes issue 135 for whatever language `stuck --next` names next,
in this same shape, or the report says `stuck --next` printed nothing and the chain has ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `tel`.
- One live `retry --language tel` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Telugu `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 135 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Telugu in this quest's live run. Changing the
