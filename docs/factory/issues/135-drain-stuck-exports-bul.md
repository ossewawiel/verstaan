...
Issue 134 drained Telugu: `python -m tools.mirror retry --language tel` landed every `tel`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `bul` (Bulgarian), the next answer from its fallback rule once the stuck language ahead of
...
After this quest, Bulgarian is drained the same way Telugu was: `python -m tools.mirror retry
--language bul --passes 3 --pause-seconds 600` runs against the live archive until `bul` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Bulgarian `stuck` count before
and after. At its close, `/quest` writes issue 136 for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `bul`.
- One live `retry --language bul` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Bulgarian `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 136 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Bulgarian in this quest's live run. Changing
