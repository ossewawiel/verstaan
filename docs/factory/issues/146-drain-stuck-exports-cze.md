...
checkpoint: null
commit: null
worktree: .worktrees/side-146-drain-stuck-exports-cze
github_issue: 166
---
## What

Issue 145 drained Berber: `python -m tools.mirror retry --language ber` landed every `ber`
export the archive would give up, in a single pass. `python -m tools.mirror stuck --next` now
names `cze` (Czech), the next answer from its fallback rule once the stuck language ahead of
...
After this quest, Czech is drained the same way Berber was: `python -m tools.mirror retry
--language cze --passes 3 --pause-seconds 600` runs against the live archive until `cze` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Czech `stuck` count before
and after. At its close, `/quest` writes issue 147 for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `cze`.
- One live `retry --language cze` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Czech `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 147 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Czech in this quest's live run. Changing
...
- [ ] Czech is drained as far as three passes allow: each pass's summary line and the
      Czech `stuck` count before and after are in the report, and the landed zips are in
      the work commit.
- [ ] Issue 147 for the next language is on `main`, or the report records that `stuck --next`
