...
checkpoint: null
commit: null
worktree: .worktrees/side-163-drain-stuck-exports-prs
github_issue: 216
---
## What

Issue 161 drained Polish: `python -m tools.mirror retry --language pol` landed every `pol`
export the archive would give up, in a single pass. `python -m tools.mirror stuck --next` now
names `prs` (Dari, ISO1 `prs`), the next answer from its fallback rule once the stuck language
...
After this quest, Dari is drained the same way Polish was: `python -m tools.mirror retry
--language prs --passes 3 --pause-seconds 600` runs against the live archive until `prs` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Dari `stuck` count before and
after. At its close, `/quest` writes the next issue for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

Numbering note: issue 162 is already taken by an unrelated console self-restart quest, written
and pushed independently the same day. This quest is issue 163, and its `depends_on` points at
161, the last language-drain quest, not 162.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `prs`.
- One live `retry --language prs` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Dari `stuck` count before and after.
...
Retrying `error` paths. Any language other than Dari in this quest's live run. Changing
...
the console. Issue 162's console self-restart fix — unrelated work, tracked separately.

## Done when

- [ ] Dari is drained as far as three passes allow: each pass's summary line and the
      Dari `stuck` count before and after are in the report, and the landed zips are in
      the work commit.
