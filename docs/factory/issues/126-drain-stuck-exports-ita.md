...
Issue 125 drained Russian: `python -m tools.mirror retry --language rus` landed every `rus`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `ita` (Italian), the next answer from its fallback rule once the stuck language ahead of
...
After this quest, Italian is drained the same way Russian was: `python -m tools.mirror retry
--language ita --passes 3 --pause-seconds 600` runs against the live archive until `ita` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Italian `stuck` count before
and after. At its close, `/quest` writes issue 127 for whatever language `stuck --next` names
next, in this same shape, or the report says `stuck --next` printed nothing and the chain has
ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `ita`.
- One live `retry --language ita` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Italian `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 127 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Italian in this quest's live run. Changing the
