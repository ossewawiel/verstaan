...
checkpoint: null
commit: null
worktree: .worktrees/quest-136-drain-stuck-exports-may
github_issue: 141
---
## What

Issue 135 drained Bulgarian: `python -m tools.mirror retry --language bul` landed every `bul`
export the archive would give up in a single pass. `python -m tools.mirror stuck --next` now
names `may` (Malay), the next answer from its fallback rule once the stuck language ahead of it
...
After this quest, Malay is drained the same way Bulgarian was: `python -m tools.mirror retry
--language may --passes 3 --pause-seconds 600` runs against the live archive until `may` has no
`timeout` path left or the passes run out, its landed zips and manifest lines are in the work
commit, and the report carries every pass's summary line and the Malay `stuck` count before and
after. At its close, `/quest` writes issue 137 for whatever language `stuck --next` names next,
in this same shape, or the report says `stuck --next` printed nothing and the chain has ended.

## Acceptance criteria

- `python -m tools.mirror stuck --next`, run before the live retry, prints `may`.
- One live `retry --language may` run, using the `stuck --next`, `retry --language`,
  `--passes` and `--pause-seconds` machinery issue 118 built, with the report giving every
  pass's summary line and the Malay `stuck` count before and after.
- The zips that landed and their manifest lines are in the work commit.
- Issue 137 exists for the language `stuck --next` names after the live run, written with
...
Retrying `error` paths. Any language other than Malay in this quest's live run. Changing the
...
- [ ] Malay is drained as far as three passes allow: each pass's summary line and the Malay
      `stuck` count before and after are in the report, and the landed zips are in the work
      commit.
- [ ] Issue 137 for the next language is on `main`, or the report records that `stuck --next`
